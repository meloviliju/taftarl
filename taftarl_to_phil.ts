...
const phil_rout = document.getElementById("phil_rout")! as HTMLFormElement;
const phil_stujesn = document.getElementById("phil_stujesn")! as HTMLFormElement
const phil_snenik = document.getElementById("phil_snenik")! as HTMLFormElement
const phil_liestu = document.getElementById("phil_liestu")! as HTMLFormElement
const phil_rukest = document.getElementById("phil_rukest")! as HTMLFormElement
const phil_stususn = document.getElementById("phil_stususn")! as HTMLFormElement

const taftarl_cyrl = document.getElementById("taftarl_cyrl")! as HTMLFormElement
const taftarl_forl = document.getElementById("taftarl_forl")! as HTMLFormElement
const taftarl_is_leap = document.getElementById("is_leap")! as HTMLFormElement
...
const stususn_analyser = (delta: number) => {
    if (delta < 0) throw new Error("delta is needed to be positive or 0");
...
const taftarl_to_philifiar = () => {
    ...
    const { rout, stujesn, snenik, liestu, rukest, stususn } = taf_to_phil(
        Number(taftarl_cyrl.value),
        Number(taftarl_forl.value)
    );
...
const philifiar_to_taftarl = () => {
    const { cyrl, forl } = phil_to_taf(
        Number(phil_rout.value),
        Number(phil_stujesn.value),
        Number(phil_snenik.value),
        Number(phil_liestu.value),
        Number(phil_rukest.value),
        Number(phil_stususn.value)
    );
...
const calc_forl_delta = (c: number, f: number) => {
    let ret_delta = f - 1;
    let year = 0;
    if (c >= 1090) {
...
const taf_to_phil = (cyrl: number, forl: number) => {
    const ret_phil = {
        "rout": 0,
        "stujesn": 0,
        "snenik": 0,
...
const calc_stususn_delta = (rout: number, stujesn: number, snenik: number, liestu: number, rukest: number, stususn: number) => {
    if (rout >= 2016) {
...
const phil_to_taf = (rout: number, stujesn: number, snenik: number, liestu: number, rukest: number, stususn: number) => {
    const ret_taf = {
        "cyrl": 0,
        "forl": 0
    }
...
// テスト
type PhilSet = [number, number, number, number, number, number];
type TafSet = [number, number];
type StususnDelta = number;

const testcases: [PhilSet, TafSet, StususnDelta][] = [
...
const test_phil_to_taf = (phil_input: PhilSet, taftarl_output: TafSet) => {
...
const test_taf_to_phil = (taftarl_input: TafSet, phil_output: PhilSet) => {
...
const test_calc_forl_delta = (taftarl_input: TafSet, stususn_output: StususnDelta) => {
    const [cyrl, forl] = taftarl_input;
...
const test_calc_stususn_delta = (phil_input: PhilSet, stususn_output: StususnDelta) => {
    const [rout, stujesn, snenik, liestu, rukest, stususn] = phil_input;
